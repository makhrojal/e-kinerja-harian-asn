...
for(const id of ['deleteActivityBtn','deleteActivityDialog','deleteActivitySummary','cancelDeleteActivity','confirmDeleteActivity'])assert.equal((raw.match(new RegExp('id="'+id+'"','g'))||[]).length,1,id+' must be unique');
...
const sh={getParent:()=>book,getMaxRows:()=>capacity,insertRowsAfter:(n,k)=>capacity+=k,deleteRow:row=>{grid.splice(row-1,1)},getLastRow:()=>grid.length,getRange(row,col,n,m){const range={getValues(){reads++;return Array.from({length:n},(_,i)=>Array.from({length:m},(_,j)=>grid[row-1+i]?.[col-1+j]??''))},setValues(values){values.forEach((a,i)=>{grid[row-1+i]??=[];a.forEach((v,j)=>grid[row-1+i][col-1+j]=typeof v==='string'&&v.startsWith("'")?v.slice(1):v)});return range;}};['setBackground','setFontColor','setFontWeight','setNumberFormat','setWrap','setVerticalAlignment'].forEach(k=>range[k]=()=>range);return range;}};
...
const deletable={...base,id:'delete-0001',activity:'Delete fixture'};const deleteSaved=ctx.saveRecord(deletable);assert.throws(()=>ctx.deleteRecord(deletable.id,deleteSaved.version+1),/sesi lain/);const deleteResult=ctx.deleteRecord(deletable.id,deleteSaved.version);assert.equal(deleteResult.deleted,true);assert.equal(deleteResult.alreadyDeleted,false);assert.equal(ctx.getRecord(deletable.id),null);const deleteRetry=ctx.deleteRecord(deletable.id,deleteSaved.version);assert.equal(deleteRetry.alreadyDeleted,true);assert.equal(held,false);ok('Activity deletion is revision-safe, idempotent, and releases the lock');
...
fc.deleteUiRecord={id:'delete-ui-0001',version:3,date:'2026-09-09',activity:'[P1] Kegiatan yang dapat dihapus',result:'Hasil',evidence:'',status:'Selesai',followup:'',created:'2026-09-09T00:00:00Z',updated:'2026-09-09T00:00:00Z'};vm.runInContext("loadFormAttachments=()=>Promise.resolve();resetForm();openForm(deleteUiRecord);$('deleteActivityBtn').click()",fc);assert.equal($('deleteActivityBtn').hidden,false);assert.equal($('deleteActivityDialog').open,true);assert.match($('deleteActivitySummary').textContent,/Kegiatan yang dapat dihapus/);vm.runInContext('resetForm()',fc);assert.equal($('deleteActivityBtn').hidden,true);ok('Edit form exposes a contextual, explicit activity deletion confirmation');
